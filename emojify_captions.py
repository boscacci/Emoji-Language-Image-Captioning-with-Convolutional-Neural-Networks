...
w2v = gsm.KeyedVectors.load_word2vec_format(
...
SAVE_FILE_NAME = "captions_val.json".format(TOP_N_PREDICTED_EMOJIS)
                    ...
                    if s not in stop_cache:
                        ...
                        if "NN" in pos:
                            try:
                                ...
                                result.append(
                                    vec
                                )  ### We want nouns to be most influential
                            except:
                                pass
                        elif "VB" in pos:
                            try:
...
with open("annotations/captions_val2017.json") as f:
    ...
    for i in data["annotations"]:
        ...
        image_id_to_vectors_dic[i["image_id"]] += vectorize(
            i["caption"], i["image_id"]
        )
    for i in image_id_to_vectors_dic.keys():
        for vec in image_id_to_vectors_dic[i]:
            image_id_to_emojis_dic[i] += e2v.most_similar(
                topn=TOP_N_PREDICTED_EMOJIS, positive=[vec]
            )
